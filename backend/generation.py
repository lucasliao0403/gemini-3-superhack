...
def _build_grok_prompt(clip: Dict[str, Any], format_data: Dict[str, Any]) -> str:
    ...
    frame_prompts = clip_prompts.get("frame_prompts") or []
    beat_captions = clip.get("beat_captions") or []
    caption_lines: List[str] = []
    if isinstance(frame_prompts, list):
        caption_lines = [str(item).strip() for item in frame_prompts if str(item).strip()]
    if not caption_lines and isinstance(beat_captions, list) and not ignore_real_beats:
        caption_lines = [str(item).strip() for item in beat_captions if str(item).strip()]
    if not caption_lines and not ignore_real_beats:
    ...
    if format_data.get("skip_asset_extraction"):
        prompt_parts.append(
            "Freeform mode: this reel is not tied to the original footage. "
            "Feel free to invent details, exaggerate action, and deviate from realism."
        )
    if ignore_real_beats:
        prompt_parts.append(
            "Do not recreate the original clip beats; follow the generated storyboard prompts only."
        )
    if video_prompt:
    ...
    if len(prompt) > MAX_FAL_PROMPT_CHARS:
        logger.warning(
            "grok_prompt_truncated length=%s max=%s clip_id=%s format_id=%s",
            len(prompt),
            MAX_FAL_PROMPT_CHARS,
            clip.get("clip_id"),
            format_data.get("id"),
        )
        prompt = prompt[:MAX_FAL_PROMPT_CHARS]
    return prompt
    ...
    async def _run_one(clip: Dict[str, Any]) -> None:
        async with semaphore:
            ...
            try:
                ...
                video_debug = debug_prompts.get("video") or {}
                video_debug["grok_prompt"] = prompt
                debug_prompts["video"] = video_debug
                ...
                debug_prompts = clip.get("debug_prompts") or {}
                video_debug = debug_prompts.get("video") or {}
                if isinstance(fal_debug, dict):
                    video_debug["fal_payload"] = fal_debug.get("input")
                debug_prompts["video"] = video_debug
                clip["debug_prompts"] = debug_prompts
            except Exception:
                logger.exception(
